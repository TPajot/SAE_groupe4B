...
#---------------------------------------------------------#
#plateau={"nb_colonnes" = ..., "nb_lignes" = ..., "les_valeurs" = [...]}
...
def distances_objets_joueurs(plateau, pos, distance_max):
    ...
    while i < len(cases_a_dist):
        ...
        dist_courante = cases_a_dist[i][1]
        ...
        if objet_case_c != const.AUCUN:
            if dist_courante not in dico.keys():
                dico[dist_courante]=set()
            dico[dist_courante].add(objet_case_c)
        ...
        if dist_courante<distance_max:
            ...
            for direction in direction_voisines.keys():
                ...
                if pos_prochaine_case not in vues: #si je n'ai pas déjà vue la position
                    cases_a_dist.append((pos_prochaine_case,dist_courante+1))
